...
If you're using the graphs programmatically (outside of LangGraph Studio), you can invoke them using the new Context API pattern:
...
# Query the retrieval graph
response = await graph.ainvoke(
    {"messages": [("user", "What does my cat know?")]},
    context={
        "user_id": "your_user_id", 
        "retriever_provider": "elastic-local",
        "response_model": "anthropic/claude-3-5-sonnet-20240620"
...
The context dictionary can include any of the configuration parameters defined in the `Configuration` and `IndexConfiguration` classes. Parameters not provided in the context will use their default values or be populated from environment variables.
